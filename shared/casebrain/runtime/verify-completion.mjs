...
for (const task of state.tasks ?? []) {
  ...
  if (task.status === 'COMPLETE') {
    ...
    if (task.verification_result !== 'PASS') {
      errors.push(`${task.task_id}: COMPLETE requires verification_result=PASS`);
    }

    if (!Array.isArray(task.proof) || task.proof.length < 2) {
      errors.push(`${task.task_id}: COMPLETE requires at least two proof objects`);
    }
  }

  if (task.status === 'IN_PROGRESS' || task.status === 'BLOCKED' || task.status === 'EXECUTED_UNVERIFIED') {
    if (!task.next_executable_action) {
